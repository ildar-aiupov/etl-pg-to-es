...
class Postgres:
    ...
    def check_updates(self, state):
        ...
        self.person_ids = self.cursor.fetchall()
        if self.person_ids:
            self.person_ids = tuple(element["id"] for element in self.person_ids)
        return len(self.person_ids)

    def load_portion(self):
        self.cursor.execute(sql_queries.select_data, [self.person_ids])
